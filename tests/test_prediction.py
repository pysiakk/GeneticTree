...
@pytest.fixture
def genetic_tree():
    gt = GeneticTree(n_trees=n_trees, n_iters=30, is_keep_last_population=True, is_remove_variables=False)
...
if __name__ == "__main__":
    gt = GeneticTree(is_keep_last_population=True, is_remove_variables=False, n_iters=30)
    gt.fit(iris.data, iris.target)
    tree = gt._best_tree
